...
class _RefreshmentDetailsState extends State<RefreshmentDetails> {
  ...
  Map<String, dynamic> refreshmentData = {};
  ...
  Future<void> getData() async {
    ...
    try {
      refreshmentData = await refreshmentRepository.getRefreshmentDetails(date: format,mode: mode);
      foodData = await refreshmentRepository.getFoodDetails(date :format);

        setState(() {
          _milkCount = refreshmentData['milk_count'] ?? 0;
          _teaCount = refreshmentData['tea_count'] ?? 0;
          _coffeeCount = refreshmentData['coffee_count'] ?? 0;
          _foodCount = foodData['lunch_count'] ?? 0;
        });

    } catch (e) {
      Exception('Error caught while getting refreshment details! $e');
    }
    setState(() {
      _isLoading = false;
    });
  ...
  Widget buildTeaSection() {
    ...
    if (_teaCount > 0) {
      final Map<dynamic, dynamic> tea = refreshmentData['tea'];
  ...
  Widget buildCoffeeSection() {
    ...
    if (_coffeeCount > 0) {
      final Map<dynamic, dynamic> coffee = refreshmentData['coffee'];
  ...
  Widget buildMilkSection() {
    ...
    if (_milkCount > 0) {
      final Map<dynamic, dynamic> milk = refreshmentData['milk'];
  ...
  Widget buildFoodSection() {
    ...
    if (_foodCount > 0) {
      final Map<dynamic, dynamic> food = foodData['lunch_list'];
